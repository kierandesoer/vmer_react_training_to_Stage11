...
export const Person = ({person, deletePerson, editPerson}) => {
    ...
    return <div style={{ padding: 10 }} key={`${person.name}, ${person.age}`}>
        <div>
            ...
            <input disabled={!state.editing} type='text' placeholder="Enter New Name" id='editPersonName' value={state.updatePersonName} onChange={(e) => dispatch({ type: 'updatePersonName', payload: e.target.value })} />
            <label htmlFor="editPersonAge">Age: </label>
            <input disabled={!state.editing} type='number' placeholder="Entner New Age" id='editPersonAge' value={state.updatePersonAge} onChange={(e) => dispatch({ type: 'updatePersonAge', payload: e.target.value })} />
